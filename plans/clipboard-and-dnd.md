...
- **macOS native host**: walks the same tree (`Session::find_drop_target_in_frame_macos`, with `_current_drop_abs_x/y` cached for MOVE/LEAVE). `NEUINativeContentView` stays the single `<NSDraggingDestination>`; per-widget targeting is done in software (matching win32 native, where child HWNDs likewise don't register their own targets). A per-painted-view native opt-in is still possible but unnecessary for parity.
...
- **macOS** (`hosts/shared/macos/dnd_source_macos.h`): `NEUIDragSource : NSObject<NSDraggingSource>` captures the final `NSDragOperation` from `draggingSession:endedAtPoint:operation:` and flips a `done` flag. `macos_run_drag_source` builds `NSDraggingItem`s from the DataItem (one shared `NSPasteboardItem` for text/HTML/MIME, one item per URL for `text/uri-list`), calls `beginDraggingSessionWithItems:event:source:` on the frame's content view, then spins a self-contained runloop pump (`dnd_pump_until(&src->done)`) to match the synchronous Win32 contract - the native macOS host doesn't implement the xpl `platform_run_modal_until` seam, so the pump lives in the shared header. The `@implementation NEUIDragSource` is `#ifdef NEUI_DND_SOURCE_MACOS_IMPLEMENTATION`-guarded (defined only by xpl `platform_macos.mm`) so it isn't duplicated when both hosts co-link. `NSDragOperation` constants do NOT match `DROPEFFECT_*`; explicit mapping in `nsop_to_dnd_action`.
...
- Builds + runs on Win32 (native + xpl) and macOS (native + xpl, Xcode arm64). The first macOS build surfaced four issues now fixed: examples passed a title string where `create()` expects `void* userdata` (clang-hard, MSVC-lenient - use `set_text` instead); `__block` locals captured in C++ lambdas in `clipboard_macos.h` / `dnd_source_macos.h`; `NEUINativeWindowDelegate`'s ivar interface declared *after* the `NEUINativeContentView` `<NSDraggingDestination>` methods that reach through `window.delegate` (moved the interface above); and the drag-source pump + `@implementation NEUIDragSource` duplication described above.
