...
- **Native (non-xpl) Win32 host scrolling SECTION.** Scrolling SECTION has since shipped on the Win32 native host too (`painted_msg_section_w32`); see the scrolling-SECTION rationale in `docs/design-notes.md`. Phase 3 can land DM on either host's SECTION HWND - confirm the current kinetics path before wiring.
...
- Programmatic `widgets->scroll_to(widget, x, y, animate=bool)` that uses `viewport->ZoomToRect` on DM-active surfaces and falls back to direct scroll-position writes elsewhere. (The non-animated form shipped as the `NEUI_API_SCROLL` `set_scroll`/`ensure_visible`; the `animate` variant is the DM-specific follow-up.)
