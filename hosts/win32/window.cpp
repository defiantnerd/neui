...
namespace win32_host
{
  ...
  void register_classes()
  {
    ...
    wc.lpszClassName = L"neui.image";
    RegisterClassExW(&wc);

    // Shared class for self-painted widgets (knob, future meter / scope / XY pad).
    // Hosts a per-widget D2D context; paint logic plugs in via WidgetData::paint_fn.
  ...
  LRESULT CALLBACK ImageWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
  {
    ...
    case WM_CREATE: {
      if (wd && backend) {
        ...
        wd->image_ctx = backend->create_context(hwnd,
    ...
    case WM_SIZE: {
      if (wd && backend && wd->image_ctx) {
        backend->resize(wd->image_ctx,
    ...
    case WM_SHOWWINDOW:
      // Ensure repaint when the window becomes visible (directly or because
      // the parent frame was shown after the bitmap was already loaded).
      if (wParam) InvalidateRect(hwnd, nullptr, FALSE);
      return DefWindowProcW(hwnd, msg, wParam, lParam);
    case WM_ERASEBKGND:
      return 1;  // background handled in WM_PAINT via D2D
    case WM_PAINT: {
      PAINTSTRUCT ps;
      BeginPaint(hwnd, &ps);
      EndPaint(hwnd, &ps);
      ...
      if (wd && backend && wd->image_ctx) {
        ...
        if (wd->image_bitmap) {
          ...
          float w_log = static_cast<float>(rc.right)  * 96.0f / static_cast<float>(dpi);
          float h_log = static_cast<float>(rc.bottom) * 96.0f / static_cast<float>(dpi);
          ...
          if (img_w_log > 0.0f && img_h_log > 0.0f) {
            ...
            if (image_aspect > widget_aspect) {
              dst_w = w_log;
              dst_h = w_log / image_aspect;
              dst_x = 0.0f;
              dst_y = (h_log - dst_h) * 0.5f;
            } else {
              dst_h = h_log;
              dst_w = h_log * image_aspect;
              dst_y = 0.0f;
              dst_x = (w_log - dst_w) * 0.5f;
            }
          }

          // Optional rotation around the destination centre.
          ...
          if (rotated) {
            ...
            backend->push_transform(wd->image_ctx);
            backend->translate(wd->image_ctx,  cx,  cy);
            backend->rotate   (wd->image_ctx, rot);
            backend->translate(wd->image_ctx, -cx, -cy);
          }
          ...
          if (rotated) backend->pop_transform(wd->image_ctx);
        }
        backend->end_frame(wd->image_ctx);
      }
      return 0;
    }
    case WM_DESTROY: {
