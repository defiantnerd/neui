...
- **Named interface dispatch** - `get_interface(sess, name)` with version suffix (`/0`). Active: `NEUI_API_WIDGETS/_ITEMS/_TREE/_ATTRS/_CLIPBOARD/_COMMANDS/_ASSETS/_COMPOUND/_BEHAVIOR/_GRID`. Optional client-side: `_CLIPBOARD_CLIENT`, `_MENU_CLIENT`, `_THEME_CLIENT`, `_GRID_CLIENT`.
...
`plans/`: `painter-and-asset-api.md` (compound + asset, shipped), `grid-macos-port.md` (GRID macOS port, shipped), `grid-sorting.md` (multi-column sort, shipped), `winui3-host.md` (deferred), `wasm-host.md` (deferred), `how-to-port.md` (new-platform playbook).
