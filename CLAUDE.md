...
- **DnD drag-source extensions** - drag-source API, the `DRAG_SOURCE` behavior-asset handler kind, and the custom drag-preview image (`begin_drag_with_preview`; `NSImage` / `IDragSourceHelper`) all ship on all three hosts; deferred follow-ups: lazy promise data (`CFSTR_FILECONTENTS` / `NSFilePromiseProvider`), per-drag result feedback through `emit_attr_changed` (currently the `DRAG_SOURCE` handler drops the negotiated action on the floor).
...
`plans/`: `painter-and-asset-api.md` (compound + asset, shipped), `grid-macos-port.md` (GRID macOS port, shipped), `grid-sorting.md` (multi-column sort, shipped), `clipboard-and-dnd.md` (unified data-item + clipboard formats + DnD drop-target, shipped), `dnd-drag-source-fixes.md` (drag-source parity + macOS safety, shipped), `render-to-surface.md` (SURFACE asset kind, shipped), `winui3-host.md` (deferred), `wasm-host.md` (deferred), `how-to-port.md` (new-platform playbook), `crossplatform-host-sketch.md` (early xpl design sketch, archival - superseded by the shipped impl).
