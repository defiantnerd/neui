...
Outputs - Windows: `out/build/Debug/{neui_example.exe, neui.lib, neui-win32host.lib, neui-xplhost.lib, neui-backend-d2d.lib}`. macOS: `out/build/Debug/{neui_example.app, libneui.a}` + per-subdir `libneui-*.a`. Example apps (CMake targets): `neui_example`, `neui_grid_example`, `neui_section_scroll_example`, `neui_surface_example`, `neui_dnd_example`, `neui_dnd_source_example`, `neui_tabview_example`, `neui_font_loading_example`, `neui_filter_knob_example`, `neui_path_example`, `neui_overlay_example` (transparent CUSTOMDRAW overlay - DirectComposition on win32), `neui_arc_example` (value-driven arc / ring / pie compound layer), `neui_zoom_example` (`NEUI_ATTR_UI_SCALE` 100/150/200 % + logical vs device-pixel CUSTOMDRAW painting), `neui_a11y_example` (`NEUI_API_A11Y` - declared roles on custom-painted controls, a normalized value declared in real dB, LABEL-names-INPUTBOX; run it with VoiceOver), `neui_popup_surface_example` (`NEUI_W_POPUPSURFACE` - a 640x520 picker opened from a 520x300 window, so it visibly leaves the frame; cascade, AT_POINTER, dismissal reasons), and `readme_example`.

**Cross-platform build hygiene**: libc++ (macOS) pulls `<cstring>` / `<cstdio>` / `<cmath>` in transitively, **libstdc++ (GCC/Linux) does not** - so a file that calls `strlen` or `std::fabs` without including the header builds clean here and breaks the Linux CI build, invisibly to every local check. `tests/parse_check/check_includes.py` flags that class over the Linux-compiled files; run it after touching Tier-1 tests or `hosts/shared`. `tests/parse_check/run.sh` is the companion for the Windows-only `a11y_win32.cpp` (parses it against stub headers, so that file can be edited from macOS / Linux without waiting for CI - it is a typo-and-signature net only, never an oracle). **Neither is a substitute for reading CI** - Linux is the one platform with no local build here.

**Tests**: `tests/` is a Tier-1 header-only unit suite (`neui_tests`) over the portable logic in `hosts/shared/*.h` - links no host and no backend, builds everywhere including the null platform. Toggle with `-DNEUI_BUILD_TESTS=OFF`. Run directly or via `ctest --test-dir out/build -C Debug`. Linux-only extra targets: `neui_cairo_smoke` (offscreen Cairo, ctest-registered), `neui_embed_smoke` (fake-DAW embedding, needs a live X display) and `neui_popup_surface_smoke_linux` (popup surfaces - asks **X11** over a SECOND X connection, so it can act as a different X client: the popup is an `override_redirect` direct child of the root window whose rect leaves the owner, the host's `XGrabPointer` is really held, the owner keeps the input focus, and another client's keyboard grab must NOT dismiss the stack). (Historic trap, fixed 2026-08-14: an app that returned from `main()` without `api->destroy(session)` used to segfault in static destruction, because `~Session` reaches `g_windows` in `platform_linux.cpp` and cross-TU static teardown order is unspecified - `g_windows` is now immortal like `client_timer_sessions()`. The crash ate the unflushed stdout, so three fully passing harnesses looked like they printed nothing at all. If a Linux harness ever goes silent again, re-run it under `stdbuf -o0` before believing it produced no output.) **Windows-only**: `neui_focus_smoke_win32` (26 checks over five modal / frame-teardown routes, with a delay-free debug heap and a self-symbolizing crash reporter), `neui_a11y_provider_smoke_win32` (57 checks through the real UIA client stack on a separate MTA thread), and `neui_popup_surface_smoke_win32` (popup surfaces - asks **win32** rather than neui: `GetWindow(GW_OWNER)`, the `GWLP_HWNDPARENT` re-own, and `GetForegroundWindow`/`GetActiveWindow`/`GetFocus` around a real open and a real click, because on win32 the style bits are not the promise - `WS_EX_NOACTIVATE` only blocks activation-by-*click*) - all three realize real HWNDs and post real input, so they are **built but not ctest-registered**; run `tests/<config>/neui_*.exe` by hand. The macOS harnesses (`tests/*_macos.mm`) are the same shape: built, not registered, run by hand - note `neui_popup_surface_smoke_macos` (popup surfaces) asks **AppKit** rather than neui, since the feature's whole claim is about a real window: that the panel's rect leaves the owner, that it is in the owner's `-childWindows`, and that it can never become key. And note `neui_modal_smoke_macos` (modal-dialog lifetime, both macOS hosts) re-execs itself under **Guard Malloc** and runs each host in its own process, because the defects it covers are reads of freed memory that pass under a normal allocator.
...
- **Crossplatform Host** `hosts/crossplatform/`: polymorphic widget hierarchy (`neui.host.crossplatform`). `host.{h,cpp}` `WidgetData` base + per-type subclasses (`FrameWidget` … `GridWidget`) with virtuals; `widgets.cpp` full API + `make_widget()`; `platform.h` cross-cutting seam (window / menubar / image / clipboard / IME / modal / focus); per-OS impls `platform_{win32.cpp,macos.mm,linux.cpp,null.cpp}`; `a11y_adapter.{h,cpp}` turns one frame's live widget tree into the `A11yInput` rows `hosts/shared/a11y_tree.h` builds a provider tree from; `a11y_win32.{h,cpp}` is the **UI Automation provider** (verified programmatically 2026-08-11 through the real UIA client stack - `tests/a11y_provider_smoke_win32.cpp`; **no screen-reader pass yet**, see `docs/accessibility.md`), `a11y_macos.{h,mm}` is the **NSAccessibility provider** that publishes that tree to VoiceOver. **Linux AT-SPI is not written**, so there a client's declarations are stored and nothing reads them (same on iOS / null, and on the native hosts - `NEUI_API_A11Y` is xpl-only). See `plans/accessibility.md`.
...
- **Named interface dispatch** - `get_interface(sess, name)` with version suffix (`/0`). Active: `NEUI_API_WIDGETS/_ITEMS/_TREE/_ATTRS/_CLIPBOARD/_DND/_COMMANDS/_ASSETS/_COMPOUND/_BEHAVIOR/_FILTER/_GRID/_SCROLL/_NOTIFY/_EMBED/_TIMER/_POINTER/_A11Y`. Optional client-side: `_MENU_CLIENT`, `_THEME_CLIENT`, `_GRID_CLIENT`. `_FILTER` is itself optional host-side (a host without off-screen surfaces may return nullptr); `_EMBED` (`d/embed.h`, DAW embedding for PLUGWINDOW), `_TIMER` (`d/timer.h`), `_POINTER` (`d/pointer.h`, relative/unbounded pointer mode so a knob drag survives the screen edge), `_POPUP` (`d/popup.h`, popup surfaces that may leave the frame - see `docs/popup-surfaces.md`) and `_A11Y` (`d/a11y.h`, accessibility declarations - see `plans/accessibility.md`) are exposed by the **xpl host only** - note that on win32/macOS `neui_get_api(NULL)` returns the NATIVE host first, so a client taking the default there gets nullptr for all three.
...
- **Event routing** - host -> client via `neui_widget_client_t::onevent()`. Client gets first chance for mouse events; false forwards to `widget->on_mouse_event()`. **A client handler may destroy widgets - including the one being dispatched to - and still return false**: `dispatch_mouse_event` / `dispatch_wheel_event` re-resolve the target from the slot table after the callback rather than calling through the reference they started with (a destroyed widget simply has no built-in handling left to run). "The OK button was clicked, tear the dialog down" plus the documented `return false` default is the most ordinary handler there is, and it used to be a use-after-free.
...
| `POPUPSURFACE` | A frameless overlay that may leave its owner frame - the third FRAME kind after APPWINDOW / PLUGWINDOW, so it owns a coordinate origin, a render surface and a child subtree, and ordinary widgets work inside it. Created with `parent = widget_none`; **`widgets->show` is ignored** (it has no position until anchored) - `NEUI_API_POPUP::open` places and shows it against an anchor + offset + side, flipping and clamping to the monitor work area. Where the platform has owned top-level windows the host backs it with a real borderless non-activating window, so a 1030x970 picker opened from a 700x500 plugin editor hangs over the DAW; elsewhere (iOS / null) it falls back to an in-frame overlay clamped to the owner (**not written yet** - `open()` fails honestly there). All three desktop backings (macOS, win32, Linux/X11) are verified, each by a harness that asks the PLATFORM rather than neui. See `docs/popup-surfaces.md`. |
| `POPUPMENU` | Model for `widgets->popup_tree_menu` (xpl host only) - a context menu described with the same `NEUI_API_TREE` calls as a MENUBAR, so submenus / checkmarks / enable / shortcut labels / `set_menu_cmd` / `validate` all work. **Not** a menu bar: never attached to the frame, no in-frame band, and a frame may carry both. No size or visual presence of its own. Async - the pick arrives as one `ITEM_SELECTED` on the menu widget. See `docs/menus-commands-dialogs.md` for the `is_menubar()` / `is_menu_model()` / `uses_native_menu()` split, which is load-bearing. |
...
| `KNOB` | Painted rotary. **Wheel: up DECREASES, down increases** - the audio-plugin convention, shared with SLIDER and with a behavior asset's `WHEEL` handler via `hosts/shared/wheel_direction.h::wheel_value_sign` (one place; it used to be five, and they disagreed). Right-click -> "Reset to default" popup (`NEUI_PARAM_DEFAULT`); drag mode via `NEUI_ATTR_KNOB_MODE` (rotational / vertical / horizontal); fill anchor via `NEUI_ATTR_POLARITY`; `NEUI_ATTR_VALUE_TEXT` overlay; `NEUI_ATTR_STEPS` detents. Not a keyboard tab-stop on macOS (deferred). |
...
- **Popup surfaces that leave the frame** -> `docs/popup-surfaces.md`. `NEUI_W_POPUPSURFACE` + `NEUI_API_POPUP`: the third FRAME kind, the two backings (own OS window vs in-frame overlay), anchor/side placement with work-area clamp + flip, the session input gate that replaces a pointer capture, and dismissal. macOS, win32 and Linux/X11 are all verified (each found exactly one platform-specific defect on its first run) - read the per-platform status table before assuming a backing works where you are.
...
- **Accessibility** -> `docs/accessibility.md`. `NEUI_API_A11Y`, the three-layer tree (adapter -> portable model -> platform provider), per-platform status. **Read the status table first**: macOS is verified against VoiceOver; the win32 UIA provider is verified *programmatically* (real UIA client stack) but **has never been heard by a screen reader**; Linux has no provider at all.
