...
| `NEUI_W_POPUPMENU` | Model-only widget for `widgets->popup_tree_menu` (tree-driven context menu). Shares `MenubarWidget` wholesale - same tree API, same cascade layout/paint - but is never a frame's menu band, so it reserves no space and paints nothing until shown. Async pick via `ITEM_SELECTED`. xpl host only. See `docs/menus-commands-dialogs.md`. |
