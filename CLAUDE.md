...
- **Popup surfaces that leave the frame** -> `docs/popup-surfaces.md`. `NEUI_W_POPUPSURFACE` + `NEUI_API_POPUP`: the third FRAME kind, the two backings (own OS window vs in-frame overlay), anchor/side placement with work-area clamp + flip, the session input gate that replaces a pointer capture, and dismissal. macOS and win32 are verified; **Linux is written but unbuilt** - read the per-platform status table before assuming it works where you are.
