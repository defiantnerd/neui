...
- **Accessibility** -> `docs/accessibility.md`. `NEUI_API_A11Y`, the three-layer tree (adapter -> portable model -> platform provider), per-platform status. **Read the status table first**: macOS is verified against VoiceOver, **the win32 UIA provider has never been compiled or run**, and Linux has no provider at all.
