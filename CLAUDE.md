...
`plans/`: `painter-and-asset-api.md` (compound + asset, shipped), `grid-macos-port.md` (GRID macOS port, shipped), `grid-sorting.md` (multi-column sort, shipped), `clipboard-and-dnd.md` (unified data-item + clipboard formats + DnD drop-target, shipped), `dnd-drag-source-fixes.md` (drag-source parity + macOS safety, shipped), `winui3-host.md` (deferred), `wasm-host.md` (deferred), `how-to-port.md` (new-platform playbook), `crossplatform-host-sketch.md` (early xpl design sketch, archival - superseded by the shipped impl).
