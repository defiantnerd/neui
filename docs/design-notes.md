...
- **Client-built `fe*` graph over hardcoded effect recipes**: the first cut shipped two baked C++ ops (`surface_blur` / `surface_drop_shadow`) that froze one composition of primitives. But a designer changes the `<filter>` definition freely, so neui exposes the SVG filter *model* - an ordered graph of atomic primitives the client wires (`add_primitive` / `set_input` / `set_result` / `set_*`) - and the client re-describes the graph instead of waiting for a new C++ entry point. The two convenience ops are kept but demoted to thin `filter_build_*` recipes over the same engine.
- **CPU on the baked SURFACE buffer, NOT backend-native**: this is the load-bearing decision and a deliberate reversal of the tint section's "migrate D2D to the effects framework to unlock shadow/blur." Surfaces are baked once (startup / theme / DPI change), never filtered per frame, so the GPU-speed argument evaporates; a CPU engine over `AssetEntry::pixels` gives **pixel-identical output across D2D / CG / Cairo** (a UI framework wants identical chrome everywhere) with **zero `neui_render_backend_t` ABI change** and no three-way divergence (D2D effects vs CG `CGContextSetShadowWithColor`/CIFilter vs Cairo software). The D2D effects framework stays available for any future *per-frame* need; baked filters just don't need it.
- **Optional host extension**: `NEUI_API_FILTER` may return `nullptr` (embedded hosts, or any without off-screen surfaces) - clients null-check and skip, the same graceful-degradation contract as `create_surface` → `asset_none`. The pure engine (`evaluate_filter`) is host-independent, so its Tier-1 tests run everywhere regardless of host support.
- **Premultiplied intermediates, un-premultiply only in `feColorMatrix`**: matches the SVG spec (blur/offset/composite/blend/merge/flood are premultiplied; color-matrix/component-transfer operate on straight RGBA). 8-bit intermediates (not float) match the surface buffer + the existing blur primitive - accepted minor precision loss over ~handful-of-primitive graphs.
- **sRGB, linearRGB deferred**: the designer's filters declare `color-interpolation-filters="sRGB"`, so sRGB math covers the real input and avoids per-op linearize/delinearize; a per-filter linearRGB flag is the documented future knob.
- **Mirrors the COMPOUND/BEHAVIOR asset+interface shape**: `FilterAsset` stores like `CompoundAsset`, `NEUI_API_FILTER` is vtable-shaped like `NEUI_API_COMPOUND`, applied to a SURFACE via `apply_filter` (a filter is *not* attached to a widget). One `apply_filter_asset` tail backs both `apply_filter` and every convenience wrapper.
- **Inner-shadow / bevel via the arithmetic-composite band trick**: `feComposite operator="arithmetic"` (k2=−1, k3=1) computes `SourceAlpha − offset-blur` for the inset band, then `feComposite "in"` clips it to the shape - so inset effects need no `feMorphology`. Outline/stroke is the one surveyed effect deliberately left out *because* it needs `feMorphology` (the trigger to implement that primitive later).
...
- **64-bit XID doesn't fit the int/string AttrBag cleanly, so embedding uses dedicated seams, not attrs.** The seams are fronted by the public `NEUI_API_EMBED` interface (`d/embed.h`), whose `set_parent` takes the native parent as `void*` - HWND on win32, NSView* on macOS, the XID cast through `uintptr_t` on Linux - so one adapter loop works on every platform. On win32/macOS the embedded frame is an ordinary WS_CHILD HWND / NEUIView subview, so the DAW's own pump services it: `event_fd` is -1 and `pump_and_tick` a deliberate no-op there, kept so the adapter's Linux code path needs no #ifdefs. On macOS the embedded frame has no NSWindow of its own - window-level ops (title, pos, close, constraints, activate) are no-ops so neui can never retitle/move/close the DAW's window.
...
## UI zoom (NEUI_ATTR_UI_SCALE)

- **Zoom as a renderer transform, not a change to any logical number.** One `push_transform` + `scale(Z,Z)` wraps the entire frame paint in `Session::paint_frame`. Rejected the alternative of folding Z into the context DPI (`update_dpi(dpi*Z)`): it works on D2D and Cairo but **not** on CoreGraphics, where a *window* context applies no CTM scale of its own at all (AppKit hands `drawRect:` an already-point-based context), so macOS would simply not zoom. The transform is also the only option that needs zero backend changes.
- **The client's coordinate system deliberately does not change.** Widget geometry, `get_client_rect`, mouse coordinates and the painter stay logical px at 96 DPI at every zoom, so a layout is written once and is correct at every zoom - and a client that never sets the attr is bit-for-bit unaffected. This works because `measure_text` is transform- *and* DPI-independent on all three backends: every rect derived from text (combo drop width, menubar columns, popup, toast) lands in the same logical space as the cached `abs_x/abs_y` and the logical mouse coords, so paint and hit-test agree for free with no second coordinate space to keep in sync.
- **Keep Z out of `wd.width/height` and the menubar inset.** Scaling those instead would desynchronise the inset subtraction, the `MENUBAR_BAND_H` hit-tests and the client rect. Z lives in exactly two places: the paint transform, and the platform layer's logical↔native conversions.
- **Font caches need no work.** All three backends key on `(family, weight, size)` with no DPI in the key, and rasterise through the CTM - so a transform-based zoom invalidates nothing.
- **One conversion constant per platform layer** (`WidgetData::logical_to_physical()` = DPI ratio x zoom; win32's `phys_to_log(v, frame)` overload, Linux's `window_scale(lw)`, macOS's `-frameZoom`), replacing the previously open-coded `dpi/96` expressions. macOS is the only platform where the zoom is the *only* factor, since AppKit points already equal logical px.
- **`get_scale_factor` had to stop lying.** The painter carries an `extra_scale` (the host-pushed zoom the backend cannot see) so "physical pixels per unit I draw" stays truthful - it is used for hairline alignment, bake resolution, and pixel-snapping (the QR compound layer bakes and snaps to the device grid, and would otherwise bake at 1/Z the needed resolution).
- **Device-pixel painting is a per-widget opt-in** (`NEUI_ATTR_PAINT_DEVICE_PIXELS`) rather than the default, so the common case stays "write one layout, get every zoom free" while meters / analysers / scopes can still be pixel-exact. Implemented by undoing only the zoom we pushed, inside the widget's clip, and reporting pre-multiplied dimensions.
- **The zoom is per-frame, not per-session**, because a plugin editor instance is a frame and each should be independently zoomable; also, `neui_attr_api_t` has no `set_session_float`, so a session-level knob would be an ABI change.
