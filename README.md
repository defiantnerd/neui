...
User-resizable plugin UIs are covered by `NEUI_ATTR_UI_SCALE` on the frame - a zoom (100 % / 150 % / 200 % / anything in 0.25..8.0) applied on top of the monitor's DPI. Your layout and painting stay in logical pixels at 96 DPI at every zoom level, so it is written once; the framework scales geometry, text and images and scales input back down. A `NEUI_W_CUSTOMDRAW` that needs the real device grid reads `neui_event_paint_t::scale`, or opts into device-pixel painting with `NEUI_ATTR_PAINT_DEVICE_PIXELS` (for meters, analysers, scopes). See `examples/zoom_example.cpp`.
